...
def client_data(request):
    ...
    if request.method == 'POST':
        try:
            ...
            data_verification_obj = DataVerificationHandle(response=response.__dict__,
                                                           hostname=hostname,
            ...
            # 触发器检测
            host_obj = models.Host.objects.filter(hostname=hostname).first()  # 获取主机实例
            trigger_obj_set = get_application_trigger_obj_set(application_name=application_name)   # 获取应用集对应触发器集合
            data_handle_obj = DataHandle(redis_obj=REDIS_OBJ)   # 实例化数据处理
            for trigger_obj in trigger_obj_set:    # 循环每个触发器
                data_handle_obj.load_application_data_and_calculating(host_obj=host_obj, trigger_obj=trigger_obj)  # 加载应用集数据并计算
